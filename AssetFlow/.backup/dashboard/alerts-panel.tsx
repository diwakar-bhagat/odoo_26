...
export function AlertsPanel() {
  return (
    <Card className="h-full bg-surface-1/50 backdrop-blur-md border-border/50">
      <CardHeader>
        <CardTitle className="text-lg font-medium text-foreground">Action Needed</CardTitle>
      </CardHeader>
          ...
          <div className="space-y-4">
            {alerts.map((alert, index) => (
              <motion.div
                ...
                className="flex flex-col space-y-2 p-3 rounded-lg border border-border/50 bg-background/50 hover:bg-surface-2/50 transition-colors cursor-pointer"
              >
                <div className="flex items-start justify-between gap-2">
                  <span className="font-medium text-sm text-foreground leading-tight">{alert.title}</span>
                  <Badge
                    variant="outline"
                    className={`text-[10px] uppercase px-1.5 py-0 rounded-sm font-semibold tracking-wider ${getStatusColor(alert.status)}`}
                  >
                    {alert.status}
                  </Badge>
                </div>
