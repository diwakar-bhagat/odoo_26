...
export function OrganizationTabs({ data }: { data: { departments: any[]; employees: any[] } }) {
  ...
  const filteredDepts = data.departments.filter((d) => d.name.toLowerCase().includes(search.toLowerCase()));

  const filteredEmps = data.employees.filter(
                    ...
                    filteredEmps.map((e) => (
                      ...
                      >
                        ...
                        <td className="p-4 align-middle">
                          <div
                            className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${e.role === "Admin" ? "border-transparent bg-destructive text-destructive-foreground" : e.role === "Asset Manager" ? "border-transparent bg-amber-500 text-white" : e.role === "Department Head" ? "border-transparent bg-blue-500 text-white" : "bg-secondary text-secondary-foreground"}`}
                          >
                            {e.role}
                          </div>
                        </td>
